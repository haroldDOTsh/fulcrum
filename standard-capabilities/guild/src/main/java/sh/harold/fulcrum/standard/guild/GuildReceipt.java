...
public record GuildReceipt(
        boolean accepted,
        Optional<GuildRosterSnapshot> snapshot,
    ...
    public GuildReceipt {
        ...
        if (accepted && (snapshot.isEmpty() || revision.isEmpty())) {
            throw new IllegalArgumentException("accepted guild receipt requires snapshot and revision");
        }
        if (!accepted && rejectionReason.isEmpty()) {
            throw new IllegalArgumentException("rejected guild receipt requires reason");
        }
    }

    public static GuildReceipt accepted(
            GuildRosterSnapshot snapshot,
            Revision revision,
            long fencingEpoch,
            String idempotencyKey,
            String commandId) {
        return new GuildReceipt(
                true,
    ...
    public static GuildReceipt rejected(String reason) {
        return new GuildReceipt(false, Optional.empty(), Optional.empty(), -1, "", "", Optional.of(requireNonBlank(reason, "reason")));
    }

    public String wireValue() {
        return accepted
                ? "accepted|%s|%d|%d|%s|%s".formatted(
                        snapshot.orElseThrow().guildId().value(),
