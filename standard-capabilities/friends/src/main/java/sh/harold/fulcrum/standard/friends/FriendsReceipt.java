...
public record FriendsReceipt(
        boolean accepted,
        Optional<FriendConnectionSnapshot> snapshot,
        Optional<Revision> revision,
        long fencingEpoch,
        String idempotencyKey,
        String commandId,
        Optional<String> rejectionReason) {
    public FriendsReceipt {
        ...
        if (accepted && (snapshot.isEmpty() || revision.isEmpty())) {
            throw new IllegalArgumentException("accepted friends receipt requires snapshot and revision");
        }
        if (!accepted && rejectionReason.isEmpty()) {
            throw new IllegalArgumentException("rejected friends receipt requires reason");
        }
    }

    public static FriendsReceipt accepted(
            FriendConnectionSnapshot snapshot,
            Revision revision,
            long fencingEpoch,
            String idempotencyKey,
            String commandId) {
        return new FriendsReceipt(
                true,
    ...
    public static FriendsReceipt rejected(String reason) {
        return new FriendsReceipt(false, Optional.empty(), Optional.empty(), -1, "", "", Optional.of(requireNonBlank(reason, "reason")));
    }

    public String wireValue() {
        return accepted
                ? "accepted|%s|%d|%d|%s|%s".formatted(
                        snapshot.orElseThrow().connectionId().value(),
