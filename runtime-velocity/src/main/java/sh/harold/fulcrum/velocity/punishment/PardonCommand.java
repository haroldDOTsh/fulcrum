...
public final class PardonCommand implements SimpleCommand {
    ...
    public void execute(Invocation invocation) {
        ...
        if (args.length < 1) {
            source.sendMessage(text("Usage: /pardon <punishment-id> [note]", NamedTextColor.RED));
            return;
        }
        ...
        if (!(source instanceof Player staff)) {
            source.sendMessage(text("Only in-game staff can pardon punishments.", NamedTextColor.RED));
            return;
        }
        ...
        hasRankOrHigher(source, Rank.STAFF)
                .whenComplete((allowed, throwable) -> {
                    ...
                    if (!Boolean.TRUE.equals(allowed)) {
                        proxy.getScheduler().buildTask(plugin, () ->
                                source.sendMessage(text("You must be STAFF or higher to use /pardon.", NamedTextColor.RED))).schedule();
                        return;
                    }
    ...
    private void executeAuthorized(Player staff, UUID punishmentId, String note) {
        ...
        message.setStatus(PunishmentStatus.PARDONED);
        ...
        try {
            ...
            staff.sendMessage(text("Pardon submitted for punishment " + punishmentId + ".", NamedTextColor.GREEN));
        } catch (Exception ex) {
