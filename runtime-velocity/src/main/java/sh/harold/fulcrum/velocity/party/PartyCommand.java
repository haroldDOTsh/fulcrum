...
final class PartyCommand implements SimpleCommand {
    ...
    public void execute(Invocation invocation) {
        ...
        switch (sub) {
            ...
            case "promote" -> {
                if (args.length < 2) {
                    player.sendMessage(Component.text("Usage: /party promote <player>", NamedTextColor.RED));
                    return;
                }
            ...
            case "demote" -> {
                if (args.length < 2) {
                    player.sendMessage(Component.text("Usage: /party demote <player>", NamedTextColor.RED));
                    return;
                }
            ...
            case "transfer" -> {
                if (args.length < 2) {
                    player.sendMessage(Component.text("Usage: /party transfer <player>", NamedTextColor.RED));
                    return;
                }
    ...
    private void handleCreate(Player player) {
        ...
        if (result.isSuccess()) {
            player.sendMessage(Component.text("Created a new party.", NamedTextColor.GREEN));
        } else {
    ...
    private void handleInvite(Player player, String targetName) {
        ...
        if (result.isSuccess()) {
            player.sendMessage(Component.text("Invited " + target.getUsername() + " to your party.", NamedTextColor.GREEN));
            result.invite().ifPresent(invite -> {
                long seconds = Math.max(1, (invite.getExpiresAt() - System.currentTimeMillis()) / 1000L);
                Component message = Component.text(player.getUsername() + " invited you to their party. Use /party accept to join. ("
                        + seconds + "s)", NamedTextColor.AQUA);
    ...
    private void handleAccept(Player player) {
        PartyOperationResult result = partyService.acceptInvite(player.getUniqueId(), player.getUsername());
        if (result.isSuccess()) {
    ...
    private void handleDeny(Player player) {
        partyService.declineInvite(player.getUniqueId());
    ...
    private void handleList(Player player) {
        ...
        player.sendMessage(Component.text("Party Members (" + snapshot.getSize() + "/" + PartyConstants.HARD_SIZE_CAP + "):",
                NamedTextColor.AQUA));
        snapshot.getMembers().values().stream()
                .sorted(Comparator.comparing(PartyMember::getRole)
                        .thenComparing(PartyMember::getJoinedAt))
                .forEach(member -> player.sendMessage(formatMemberLine(member)));

        if (!snapshot.getInvites().isEmpty()) {
            player.sendMessage(Component.text("Pending invites:", NamedTextColor.GRAY));
            for (PartyInvite invite : snapshot.getInvites().values()) {
                ...
                player.sendMessage(Component.text("- " + invite.getTargetUsername() + " (" + remaining + "s)", NamedTextColor.GRAY));
            }
        }
    }
    ...
    private void handleDisband(Player player) {
        ...
        if (result.isSuccess()) {
            player.sendMessage(Component.text("Disbanded your party.", NamedTextColor.YELLOW));
        } else {
            sendError(player, result);
        }
    }
    ...
    private void handleRoleChange(Player player, String targetName, RoleChange change) {
        ...
        if (result.isSuccess()) {
            String message = change == RoleChange.PROMOTE
                    ? "Promoted " + target.getUsername() + " to party moderator."
                    : "Demoted " + target.getUsername() + " to party member.";
    ...
    private void handleTransfer(Player player, String targetName) {
        ...
        if (result.isSuccess()) {
            player.sendMessage(Component.text("Transferred party leadership to " + target.getUsername() + ".", NamedTextColor.GREEN));
        } else {
            sendError(player, result);
        }
    }
    ...
    private void handleKick(Player player, String targetName) {
        ...
        if (result.isSuccess()) {
            player.sendMessage(Component.text("Kicked " + target.getUsername() + " from the party.", NamedTextColor.YELLOW));
        } else {
            sendError(player, result);
        }
    }
    ...
    private void handleKickOffline(Player player) {
        ...
        if (result.isSuccess()) {
            player.sendMessage(Component.text("Removed offline members exceeding the grace period.", NamedTextColor.YELLOW));
        } else {
            sendError(player, result);
        }
    }
    ...
    private void handleMute(Player player, boolean muted) {
        ...
        if (result.isSuccess()) {
            player.sendMessage(Component.text("Party chat is now " + (muted ? "muted." : "unmuted."),
    ...
    private void handleSettings(Player player) {
        ...
        PartySnapshot snapshot = snapshotOpt.get();
        player.sendMessage(Component.text("Party Settings:", NamedTextColor.AQUA));
        player.sendMessage(Component.text("- Invite Privacy: " + snapshot.getSettings().getInvitePrivacy(), NamedTextColor.GRAY));
        player.sendMessage(Component.text("- Auto Queue Accept: " + snapshot.getSettings().isAutoQueueAccept(), NamedTextColor.GRAY));
        player.sendMessage(Component.text("- Require Mod Warp Confirmation: " + snapshot.getSettings().isRequireModWarpConfirmation(), NamedTextColor.GRAY));
        player.sendMessage(Component.text("- Require In-Match Warp Confirmation: " + snapshot.getSettings().isRequireInMatchWarpConfirmation(), NamedTextColor.GRAY));
        player.sendMessage(Component.text("- Party Chat Muted: " + snapshot.getSettings().isPartyChatMuted(), NamedTextColor.GRAY));
    }
    ...
    private void handleWarp(Player player) {
        ...
        if (snapshot.getActiveReservationId() != null) {
            player.sendMessage(Component.text("Your party is already queued.", NamedTextColor.RED));
            return;
        }
        ...
        if (!isLeader) {
            if (!isModerator) {
                player.sendMessage(Component.text("Only the party leader can warp the party.", NamedTextColor.RED));
                return;
            }
            if (snapshot.getSettings().isRequireModWarpConfirmation()) {
                player.sendMessage(Component.text("A leader confirmation is required before moderators can warp.", NamedTextColor.RED));
                return;
            }
        }
        ...
        if (familyId == null || familyId.isBlank()) {
            player.sendMessage(Component.text("Unable to determine the matchmaking family for this server.", NamedTextColor.RED));
            return;
        }
        ...
        if (participants.isEmpty()) {
            player.sendMessage(Component.text("No online party members to warp.", NamedTextColor.RED));
            return;
        }
        ...
        if (!result.isSuccess()) {
            result.errorMessage().ifPresent(player::sendMessage);
            return;
        }
        ...
        if (!disallowedParticipants.isEmpty()) {
            ...
            disallowedParticipants.forEach(skipped -> skipped.sendMessage(Component.text(
        ...
        result.participants().stream()
                ...
                .forEach(member -> member.sendMessage(Component.text(
    ...
    private void sendHelp(Player player) {
        ...
        player.sendMessage(Component.text("/party invite <player> - Invite a player.", NamedTextColor.GRAY));
        player.sendMessage(Component.text("/party accept - Accept your latest invite.", NamedTextColor.GRAY));
        player.sendMessage(Component.text("/party list - Show party members.", NamedTextColor.GRAY));
        player.sendMessage(Component.text("/party leave - Leave your current party.", NamedTextColor.GRAY));
        player.sendMessage(Component.text("/party warp - Warp your party to this server.", NamedTextColor.GRAY));
        player.sendMessage(Component.text("/party disband - Disband your party.", NamedTextColor.GRAY));
        player.sendMessage(Component.text("/party promote <player> - Promote a member to moderator.", NamedTextColor.GRAY));
        player.sendMessage(Component.text("/party demote <player> - Demote a moderator to member.", NamedTextColor.GRAY));
        player.sendMessage(Component.text("/party transfer <player> - Transfer leadership.", NamedTextColor.GRAY));
        player.sendMessage(Component.text("/party kick <player> - Remove a player.", NamedTextColor.GRAY));
        player.sendMessage(Component.text("/party kickoffline - Remove offline members.", NamedTextColor.GRAY));
        player.sendMessage(Component.text("/party mute - Mute party chat.", NamedTextColor.GRAY));
    }
