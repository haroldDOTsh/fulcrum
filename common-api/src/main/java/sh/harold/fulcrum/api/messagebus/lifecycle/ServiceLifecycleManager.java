    ...
    private void setupMessageHandlers() {
        ...
        messageBus.subscribe(ChannelConstants.SERVER_REGISTRATION_RESPONSE, envelope -> {
            try {
                ...
                if (envelope.payload() instanceof com.fasterxml.jackson.databind.JsonNode) {
                ...
                } else if (envelope.payload() instanceof java.util.Map) {
                    // Direct Map handling
                    @SuppressWarnings("unchecked")
                    java.util.Map<String, Object> map = (java.util.Map<String, Object>) envelope.payload();
                    String tempId = (String) map.get("tempId");
                    if (tempId != null && tempId.equals(identity.getTempId())) {
                        ServerRegistrationResponse response = new ServerRegistrationResponse();
                        response.setTempId(tempId);
            ...
            try {
                ...
                if (envelope.payload() instanceof com.fasterxml.jackson.databind.JsonNode) {
                    ...
                    try {
                        ...
                        if (request.serverId() != null && request.serverId().equals(identity.getServiceId())) {
